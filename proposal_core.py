...
def render_html(plans, data, summary, company, mgr_name, mgr_phone, mgr_email):
    ...
    text_a = (
        ...
        "[07] 경추CT  [16] NK면역검사<br>"
        "[08] (혈액)알츠온(치매)  [17] (혈액)피검사(간염)<br>"
        "[09] (혈액)암 6종  [18] (혈액)암 8종"
    )
    text_b = (
        "[A] A그룹 2개 ⇄ B그룹 1개 변경 가능<br>"
        "[01] 전립선초음파  [07] MRA(뇌혈관) (3.0T)<br>"
        "[02] 심장초음파  [08] 뇌MRI (3.0T)<br>"
        "[03] MRI(요추) (3.0T)  [09] MRI(경추) (3.0T)<br>"
        "[04] MRI(뇌) (3.0T)  [10] (여)유방초음파<br>"
        "[05] CT(대장)  [11] (여)인유두종 바이러스 검사<br>"
        "[06] (혈액)유전자 30종"
    )
    text_c = (
        "[A] A그룹 4개 ⇄ C그룹 1개 변경 가능<br>"
        "[B] A그룹 2개 ⇄ B그룹 1개로 변경 가능<br>"
        "[01] PET-CT  [04] (여)유방MRI<br>"
        "[02] MRI(뇌+혈관) (3.0T)  [05] MRI(복부) (3.0T)<br>"
        "[03] MRI(심장) (3.0T)  "
