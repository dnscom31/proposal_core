...
from flyer_ui import extract_quote_payload, extract_flyer_from_excel, render_flyer_editor

st.set_page_config(page_title="건강검진 안내문 제작기", layout="wide")
st.title("건강검진 안내문 제작기")
st.caption("견적서 HTML/XLSX를 업로드하면 플랜명·금액·A/B/C 구성값을 자동 불러오거나, 안내문만 단독으로 제작할 수 있습니다.")

mode = st.radio(
    "시작 방식",
    ["안내문 새로 만들기", "견적서 HTML 불러오기", "견적서 엑셀 불러오기", "안내문 JSON 불러오기"],
    horizontal=True,
)
...
elif mode == "견적서 엑셀 불러오기":
    f = st.file_uploader("proposal_core에서 생성한 견적서 XLSX", type=["xlsx"])
    if f:
        loaded = extract_flyer_from_excel(f.getvalue())
        if loaded:
            data = loaded
            st.success("견적서 엑셀의 플랜명·금액·검진 구성을 자동으로 불러왔습니다.")
        else:
            st.error("이 엑셀에는 안내문 연동 데이터가 없습니다. 새 버전의 견적서 생성기에서 다시 생성한 XLSX를 사용해주세요.")
